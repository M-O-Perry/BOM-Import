...
from EVOUtil import createNewPart, openTASProgram, abreviateWords
...
class EVOBOM():
    ...
    def getAllParts(self):
        ...
        for row in range(ws.max_row - 2, 0, -1):
            ...
            description = str(ws["C" + str(row)].value)
            specs = str(ws["D" + str(row)].value) + "\n" + str(ws["E" + str(row)].value)
            vendor = str(ws["F" + str(row)].value)
            vendorPartNumber = str(ws["G" + str(row)].value)
            mfg = str(ws["H" + str(row)].value)
            mfgPartNumber = str(ws["I" + str(row)].value)
            referenced = (ws["J" + str(row)].value is not None and str(ws["J" + str(row)].value).strip() != "")
            print("Referenced: ", referenced, ws["J" + str(row)].value)
            qty = str(ws["K" + str(row)].value)
            
            description = abreviateWords(description)
            specs = abreviateWords(specs)
            
            seeifSpecsNone = specs.replace(" ", "").upper()
            seeifSpecsNone = seeifSpecsNone.replace("NONE", "")
            seeifSpecsNone = seeifSpecsNone.replace("\n", "")
            ...
            if vendor == "None":
                ...
                vendor = vendor[:4]+"50"
            if vendorPartNumber == "None":
                vendorPartNumber = ""
            if mfg == "None":
                mfg = ""
            if mfgPartNumber == "None":
                mfgPartNumber = ""
            
            print("Part Number: ", partNumber, "Referenced: ", referenced)
            if (partNumber is not None or partNumber.strip() != "") and not referenced:
                part = Part(partNumber.upper(), description, mfg=mfg, mfgNumber=mfgPartNumber, vendor=vendor, vendorNumber=vendorPartNumber, specs=specs)
